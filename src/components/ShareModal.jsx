...
export function ShareModal({ product, onClose }) {
  ...
  async function handleDownload() {
    ...
    try {
      ...
      a.href = url
      a.download = `${product.name || 'vestilo'}.jpg`
      a.click()
      ...
      <div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, zIndex: 201, background: 'var(--paper)', borderRadius: '4px 4px 0 0', borderTop: '1px solid var(--line)', maxHeight: '92vh', overflowY: 'auto', boxShadow: '0 -8px 32px rgba(21,16,10,0.18)' }}>
        ...
        <div style={{ padding: '14px 20px 10px' }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            {templates.map(t => (
              <button key={t.key} onClick={() => setTemplate(t.key)} style={{ flex: 1, padding: '8px 4px', borderRadius: 8, fontSize: 12, cursor: 'pointer', border: template === t.key ? '2px solid #1a1209' : '1px solid #e8e0d4', background: template === t.key ? '#1a1209' : '#fff', color: template === t.key ? '#f5e6c8' : '#9e8a6a', fontWeight: template === t.key ? 600 : 400, transition: 'all 0.15s' }}>
                {t.label}
              </button>
            ))}
          </div>
        ...
        <div style={{ padding: '8px 20px 16px' }}>
          ...
          <div style={{ background: '#ECE5DD', borderRadius: 8, padding: 10 }}>
            <div style={{ background: '#fff', borderRadius: '0 10px 10px 10px', padding: '10px 13px', fontSize: 13, lineHeight: 1.65, color: '#111', whiteSpace: 'pre-wrap', wordBreak: 'break-word', boxShadow: '0 1px 2px rgba(0,0,0,0.1)', fontFamily: 'system-ui, -apple-system, sans-serif', maxHeight: 240, overflowY: 'auto' }}>
              <MessagePreview text={message} />
              <div style={{ fontSize: 10, color: '#9e8a6a', textAlign: 'right', marginTop: 4 }}>{new Date().toLocaleTimeString('es-BO', { hour: '2-digit', minute: '2-digit' })}</div>
            </div>
          </div>
        </div>
        <div style={{ display: 'flex', gap: 10, padding: '0 20px 36px' }}>
          <button onClick={handleCopy} style={{ flex: 1, padding: '13px', borderRadius: 10, border: `1px solid ${copied ? '#a5d6a7' : '#e8e0d4'}`, background: copied ? '#e8f5e9' : '#fff', color: copied ? '#2e7d32' : '#1a1209', fontSize: 13, fontWeight: 600, cursor: 'pointer', transition: 'all 0.2s', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
            {copied ? '✓ Copiado' : '📋 Copiar'}
          </button>
          {photo && (
            <button onClick={handleDownload} style={{ flex: 1, padding: '13px', borderRadius: 10, border: '1px solid #e8e0d4', background: '#fff', color: '#1a1209', fontSize: 13, fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
              🖼️ Foto
            </button>
          )}
          <button onClick={handleWhatsApp} style={{ flex: 2, padding: '13px', borderRadius: 10, border: 'none', background: '#25D366', color: '#fff', fontSize: 13, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
...
function MessagePreview({ text }) {
  return (
    <>
      {text.split('\n').map((line, i, arr) => (
        <span key={i}>{parseLine(line)}{i < arr.length - 1 && <br />}</span>
      ))}
    </>
  )
}
...
function parseLine(line) {
  ...
  return parts.map((part, i) => {
    ...
    if (part.startsWith('http')) return <a key={i} href={part} target="_blank" rel="noopener noreferrer" style={{ color: '#128C7E', textDecoration: 'underline' }}>{part}</a>
    return part
  })
}
