...
import { getProducts, updateProduct, deleteProduct } from '../lib/supabase.js'
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#faf8f5' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, minmax(0, 1fr))', gap: 8, padding: '12px 16px' }}>
        {[
          ['Total', stats.total, '#1a1209'],
          ['Disponibles', stats.available, '#2e7d32'],
          ['Reservados', stats.reserved, '#e65100'],
          ['Vendidos', stats.sold, '#c62828'],
          ['+30 días', stats.old, '#9e8a6a'],
        ].map(([label, val, color]) => (
      ...
      <div style={{ display: 'flex', gap: 0, padding: '0 16px', background: '#fff', borderBottom: '1px solid #e8e0d4', overflowX: 'auto' }}>
        {[['all','Todos'],['available','Disponibles'],['sold','Vendidos'],['old','+30 días']].map(([key, label]) => (
      ...
      {loading ? (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))', gap: 12, padding: 16 }}>
          {filtered.map(p => (
            <ProductCard key={p.id} product={p} admin onStatusChange={handleStatusChange} />
          ))}
        </div>
      ) : (
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13, background: '#fff', borderRadius: 8, overflow: 'hidden', border: '1px solid #e8e0d4' }}>
            <thead>
              <tr style={{ background: '#1a1209', color: '#f5e6c8' }}>
                {['Producto','Talla','Precio','Ingreso','Días','Estado','Acciones'].map(h => (
